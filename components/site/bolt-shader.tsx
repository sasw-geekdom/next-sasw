...
// Domain-warped fbm flow, tinted between a dark base and the active circuit
// color, brightening toward the cursor — the "current" running through the bolt.
const FRAG = `
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      const draw = (now: number) => {
        ...
        // Ease the color toward the active circuit.
        for (let i = 0; i < 3; i++) {
    ...
    <div className="relative aspect-square w-full select-none">
      {reduce || failed ? (
        ...
        <canvas
          ...
          onTouchMove={onTouch}
          aria-hidden="true"
          className="bolt-mask absolute inset-0 h-full w-full"
        />
      )}
    </div>
  );
